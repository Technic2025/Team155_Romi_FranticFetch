...
*/
	 ...
	 * inputs outside the range 0-2 183
	 */
	/*
	...
	/**
	 * 190 * Method to score convex hull area. This scores how "complete" the
	 * particle is. Particles with large holes will score worse than a filled in
	 * shape 191
	 */
	/*
	...
	/**
	 * 198 * Method to score if the particle appears to be a trapezoid. Compares
	 * the convex hull (filled in) area to the area of the bounding box. 199 *
	 * The expectation is that the convex hull area is about 95.4% of the
	 * bounding box area for an ideal tote. 200
	 */
	/*
	...
	/**
	 * 222 * Computes the estimated distance to a target using the width of the
	 * particle in the image. For more information and graphics 223 * showing
	 * the math behind this approach see the Vision Processing section of the
	 * ScreenStepsLive documentation. 224 * 225 * @param image The image to use
	 * for measuring the particle estimated rectangle 226 * @param report The
	 * Particle Analysis Report for the particle 227 * @param isLong Boolean
	 * indicating if the target is believed to be the long side of a tote 228 * @return
	 * The estimated distance to the target in feet. 229
	 */
	/*
	...
	public void run() {
		...
		if (numParticles > 0) {
		 ...
		 tempindex=0;
			...
			horizontal= (particles.get(tempindex).BoundingRectLeft +particles.get(tempindex).BoundingRectRight)/2;
			vertical = (particles.get(tempindex).BoundingRectTop +particles.get(tempindex).BoundingRectBottom)/2;
		...
		//}
			 //
		//Timer.delay(0.0); // wait for a motor update time
	}
