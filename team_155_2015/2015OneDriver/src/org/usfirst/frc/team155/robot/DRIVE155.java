...
public class DRIVE155 {
	...
	private double foo = .75; // more of an integration scaling factor.
	...
	private void mecanum_fieldOriented() {
		myrobot.mecanumDrive_Cartesian(rightStick.getX(), rightStick.getY(), rightStick.getTwist(), roboGyro.getAngle());
	}
	...
	public void run() {
		...
		team155Mecanum_fieldOriented(rightStick.getX(), rightStick.getY(), rightStick.getX());

		// System.out.println("motorscale is "+ motorScale);
		if (rightStick.getRawButton(robotSystem.GYRO_RESET) == true)
			// PIDEnable();
			GyroReset();
