...
public class RobotContainer {
  ...
  // and change function at the very bottom as well
  ...
  private Command generateRamseteCommand() {
    ...
    String trajectoryJSON = "path/Unnamed_1.wpilib.json";
    ...
    // transform the path from the json robot centric to field centric.
    Transform2d transform = new Pose2d(Units.inchesToMeters(7.5), Units.inchesToMeters(22.5),
        Rotation2d.fromDegrees(0)).minus(trajectory.getInitialPose());
    Trajectory newTrajectory = trajectory.transformBy(transform);

    RamseteCommand ramseteCommand = new RamseteCommand(newTrajectory, m_drivetrain::getPose,
    ...
    m_drivetrain.resetOdometry(newTrajectory.getInitialPose());
   ...
   * m_drivetrain.resetOdometry(trajectory.getInitialPose()), m_drivetrain) //
   * next, we run the actual ramsete command .andThen(ramseteCommand)
   * 
   * // Finally, we make sure that the robot stops .andThen(new InstantCommand(()
   * -> m_drivetrain.tankDriveVolts(0, 0), m_drivetrain)); }
   */
  ...
  public Command getArcadeDriveCommand() {
    return new ArcadeDrive(
        ...
        // m_controller.getRawAxis(2)); //joystick
        m_drivetrain, () -> -m_controller.getRawAxis(1), () -> m_controller.getRawAxis(4)); // xbox
  }
}
